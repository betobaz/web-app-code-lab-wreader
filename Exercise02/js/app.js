...
WReader.dataController = Em.ArrayController.create({
  ...
  // Adds an item to the controller if it's no already in the controller
  addItem: function( item ){
    // Check to see if there are any items in the controller with the same item_id already
    var exists = this.filterProperty( 'item_id', item.item_id ).length;
    if( exists === 0 ){
      // If no results are returned, we insert the new item in to the data
